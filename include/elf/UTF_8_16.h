#ifndef UTF_8_16_H
#define UTF_8_16_H

//License: Public Domain
//Author: elf
//EMail: elf000@zoho.com

#include "UTF8.h"
#include "UTF16.h"

inline static U32 UTF8_BytesFrom16(const C16 *p, const C16 *pEnd) {
  U32 uBytes = 0;
  while (p != pEnd) {
      C16 c16 = *p++;
      uBytes += c16 >= 0x800? (c16 < 0x10000? 3 :  (++p, 4)) : (c16 >= 0x80? 2 : 1);
    }
  return uBytes;
}

inline static C32 UTF16_Read_UTF8_Bytes(const C16 **ppString, U8 *puUTF8Bytes) {
  const C16 *p = *ppString;
  C16 c16 = *p++;
  if (c16 >> 11 != 0x1B) {
      *ppString = p;
      *puUTF8Bytes = c16 >= 0x800? 3 : (c16 >= 0x80? 2 : 1);
      return c16;
    }
  C32 c32 = UTF16_Surrogate(c16, *p++);
  *ppString = p;
  *puUTF8Bytes = 4;
  return c32;
}

inline static C8 *UTF8_From16(C8 *p8, const C16 **pp16) {
  const C16 *p16 = *pp16;
  C32 c32 = *p16++;
  if (c32 >= 0x0800) {
      if (c32 < 0x10000) { // 1110xxxx 10xxxxxx 10xxxxxx
          //            assert(c0 < 0xD800 || c0 > 0xDFFF);
          *p8++ = 0xE0 | (c32 >> 12);
          *p8++ = 0x80 | ((c32 >> 6) & 0x3F);
          *p8++ = 0x80 | (c32 & 0x3F);
        }
      else { // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
          //            assert(c32 < 0x110000);
          c32 = (((C32)c32 - 0xD7F7) << 10) + (C32)*p16++;
          *p8++ = 0xF0 | (c32 >> 18);
          *p8++ = 0x80 | ((c32 >> 12) & 0x3F);
          *p8++ = 0x80 | ((c32 >> 6) & 0x3F);
          *p8++ = 0x80 | (c32 & 0x3F);
        }
    }
  else {
      if (c32 >= 0x80)  { // 110xxxxx 10xxxxxx
          *p8++ = 0xC0 | (c32 >> 6);
          *p8++ = 0x80 | (c32 & 0x3F);
        }
      else // 0xxxxxxx
        *p8++ = c32;
    }
  *pp16 = p16;
  return p8;
}

inline static C8 *UTF8_TryFrom16(C8 *p8, const C8 *p8End, const C16 **pp16, const C16 *p16End) {
  const C16 *p16 = *pp16;
  const C8 *p8PromisedSpaceEnd = p8End - 3;
  do {
      if (p8 < p8PromisedSpaceEnd)
        p8 = UTF8_From16(p8, &p16);
      else {
          const C16 *p16Begin = p16;
          U8 uUtf8Bytes;
          C32 code = UTF16_Read_UTF8_Bytes(&p16, &uUtf8Bytes);
          if (p8 + uUtf8Bytes > p8End) {
              *pp16 = p16Begin;
              return p8;
            }
          p8 = UTF8_Write(p8, code);
        }
    } while (p16 != p16End);
  *pp16 = p16;
  return p8;
}

inline static U32 UTF16_BytesFrom8(const C *p, const C *pEnd) {
  U32 uBytes = 0;
  while (p != pEnd) {
      C8 c8 = *p;
      if (c8 < 0xF0) {
          uBytes += 2;
          if (c8 < 0x80) // 0xxxxxxx
            ++p;
          else if (c8 < 0xE0) // 110xxxxx 10xxxxxx
            p += 2;
          else if (c8 < 0xF0) // 1110xxxx 10xxxxxx 10xxxxxx
            p += 3;
        }
      else {
          uBytes += 4;
          p += 4; // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
        }
    }
  return uBytes;
}

inline static C32 UTF8_Read_UTF16_Bytes(const C8 **ppString, U8 *puUTF16Bytes) {
  C32 c32;
  U8 uBytes;
  const C8 *p = *ppString;
  C8 c8 = *p++;
  if (c8 < 0x80) { // 0xxxxxxx
      c32 = c8;
      uBytes = 2;
    }
  else {
      if (c8 < 0xE0) { // 110xxxxx 10xxxxxx
          c32 = c8 & 0x1F;
          c32 <<= 6;
          c32 |= *p++ & 0x3F;
          uBytes = 2;
        }
      else {
          if (c8 < 0xF0) { // 1110xxxx 10xxxxxx 10xxxxxx
              c32 = c8 & 0x0F;
              c32 <<= 6;
              c32 |= *p++ & 0x3F;
              c32 <<= 6;
              c32 |= *p++ & 0x3F;
              uBytes = 2;
            }
          else { // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
              c32 = c8 & 0x07;
              c32 <<= 6;
              c32 |= *p++ & 0x3F;
              c32 <<= 6;
              c32 |= *p++ & 0x3F;
              c32 <<= 6;
              c32 |= *p++ & 0x3F;
              uBytes = 4;
            }
        }
    }
  *ppString = p;
  *puUTF16Bytes = uBytes;
  return c32;
}

inline static C32 UTF8_Read_UTF16_Size(const C8 **ppString, U8 *puUTF16) {
  C32 c32;
  U8 uSize;
  const C8 *p = *ppString;
  C8 c8 = *p++;
  if (c8 < 0x80) { // 0xxxxxxx
      c32 = c8;
      uSize = 1;
    }
  else {
      if (c8 < 0xE0) { // 110xxxxx 10xxxxxx
          c32 = c8 & 0x1F;
          uSize = 1;
        }
      else {
          if (c8 < 0xF0) { // 1110xxxx 10xxxxxx 10xxxxxx
              c32 = c8 & 0x0F;
              uSize = 1;
            }
          else { // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
              c32 = c8 & 0x07;
              c32 <<= 6;
              c32 |= *p++ & 0x3F;
              uSize = 2;
            }
          c32 <<= 6;
          c32 |= *p++ & 0x3F;
        }
      c32 <<= 6;
      c32 |= *p++ & 0x3F;
    }
  *ppString = p;
  *puUTF16 = uSize;
  return c32;
}

inline static C16 *UTF16_From8(C16 *p16, const C8 **pp8) {
  const C8 *p8 = *pp8;
  C8 c8 = *p8++;
  if (c8 >= 0xE0) {
      if (c8 < 0xF0) { // 1110xxxx 10xxxxxx 10xxxxxx
          C16 c16 = c8 & 0x0F;

          c16 <<= 6;
          c8 = *p8++;
          c8 &= 0x3F;
          c16 |= c8;

          c16 <<= 6;
          c8 = *p8++;
          c8 &= 0x3F;
          c16 |= c8;

          *p16++ = c16;
        }
      else { // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
          if (c8 < 0xF8) {
              C32 c32 = c8 & 0x07;

              c32 <<= 6;
              c8 = *p8++;
              c8 &= 0x3F;
              c32 |= c8;

              c32 <<= 6;
              c8= *p8++;
              c8 &= 0x3F;
              c32 |= c8;

              c32 <<= 6;
              c8 = *p8++;
              c8 &= 0x3F;
              c32 |= c8;

              *p16++ = (c32 >> 10) + 0xD7C0;
              *p16++ = c32 & 0x3FF | 0xDC00;
            }
          // else
          //   return 0x80000000;
        }
    }
  else if (c8 >= 0x80)  { // 110xxxxx 10xxxxxx
      C16 c16 = c8 & 0x1F;

      c16 <<= 6;
      c8 = *p8++;
      c8 &= 0x3F;
      c16 |= c8;
      *p16++ = c16;
    }
  else
    *p16++ = c8;
  *pp8 = p8;
  return p16;
}

inline static C16 *UTF16_TryFrom8(C16 *p16, const C16 *p16End, const C8 **pp8, const C8 *p8End) {
  const C16 *p16PromisedSpaceEnd = p16End - 1;
  const C8 *p8 = *pp8;
  do {
      if (p16 < p16PromisedSpaceEnd)
        p16 = UTF16_From8(p16, &p8);
      else {
          const C8 *p8Begin = p8;
          U8 uUTF16Size;
          C32 code = UTF8_Read_UTF16_Size(&p8, &uUTF16Size);
          if (p16 + uUTF16Size > p16End) {
              *pp8 = p8Begin;
              return p16;
            }
          p16 = UTF16_Write(p16, code);
        }
    } while (p8 != p8End);
  *pp8 = p8;
  return p16;
}

#endif //UTF_8_16_H
