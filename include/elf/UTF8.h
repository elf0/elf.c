...
inline static const C *UTF8_Skip(const C *p) {
  U8 u8 = *p;
  if (u8 < 0x80) // 0xxxxxxx
    return p + 1;
...
inline static const C *UTF8_SkipTail(C cHead, const C *pTail) {
...
inline static U64 UTF8_Count(const C *p, const C *pEnd) {
  U64 uCount = 0;
  while (p != pEnd) {
      ++uCount;
      C c = *p;
...
inline static C32 UTF8_ReadTail(C32 cHead, const C **ppTail) {
  const C *p = *ppTail;
  if (cHead < 0xE0) { // 110xxxxx 10xxxxxx
      cHead &= 0x1F;
      cHead <<= 6;
...
inline static C32 UTF8_Max(const C *p, const C *pEnd) {
  C32 cMax = 0;
  while (p != pEnd) {
      C32 c32 = UTF8_Read(&p);
      if (c32 > cMax)
...
inline static U8 UTF8_NeedBytes(C32 code) {
  return code < 0x80? 1 : code < 0x0800? 2 : code < 0x10000? 3 : 4;
}

//pBuffer size should >= 4 bytes
//caller must input valid utf c32
inline static C8 *UTF8_Write(C8 *pBuffer, C32 code) {
  C8 *p = pBuffer;
  if (code < 0x80) // 0xxxxxxx
    *p++ = code;
  else {
      if (code < 0x0800) { // 110xxxxx 10xxxxxx
          *p++ = 0xC0 | (code >> 6);
          *p++ = 0x80 | (code & 0x3F);
        }
      else {
          if (code < 0x10000) {//1110xxxx 10xxxxxx 10xxxxxx
...
inline static B UTF8_Valid(const C *p, const C *pEnd) {
  while (p != pEnd) {
      C32 c32 = *p++;
      // 0xxxxxxx
      if (c32 > 0x7F) {
...
inline static U64 UTF8_ParseCount(const C **ppBegin, const C *pEnd) {
  U64 uCount = 0;
  const C *p = *ppBegin;
  while (p != pEnd) {
      C32 c32 = *p++;
