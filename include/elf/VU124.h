...
//VU124 range: [0, 0xFFFF]. Check it youself!
inline
static const Byte *VU124_ToU16(const Byte *pVU124, U16 *pU16){
  const U8 *p = pVU124;
  U16 uValue = *p++;
  if(uValue > 0xF){
    U8 uTail = uValue >> 4;
...
//VU124 range: [0, 0xFFFFFFFF]. Check it youself!
inline
static const Byte *VU124_ToU32(const Byte *pVU124, U32 *pU32){
  const U8 *p = pVU124;
  U32 uValue = *p++;
  if(uValue > 0xF){
    U8 uTail = uValue >> 4;
