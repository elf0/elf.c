...
static inline E8 MMFile_Create(MMFile *pFile, const C *pPath, U32 uSize){
    File *pfFile = &pFile->file;
    if(File_Create(pfFile, pPath))
        return 1;
    pFile->pBegin = null;
...
static inline E8 MMFile_Open(MMFile *pFile, const C *pPath){
    File *pfFile = &pFile->file;
    if(File_Open(pfFile, pPath))
        return 1;
    pFile->pBegin = null;
...
static inline E8 MMFile_Map(MMFile *pFile, U64 uMinSize){
    File *pfFile = &pFile->file;
    pFile->pBegin = null;
...
static inline E8 MMFile_Prepare(MMFile *pFile, const C *pPath, U64 uMinSize){
    if(File_Prepare(&pFile->file, pPath))
        return 1;

    return MMFile_Map(pFile, uMinSize);
}

static inline E8 MMFile_MapForRead(MMFile *pFile){
    File *pfFile = &pFile->file;
    pFile->pBegin = null;
#ifdef __linux__
    if(File_ReadMeta(pfFile)){
...
static inline E8 MMFile_OpenForRead(MMFile *pFile, const C *pPath){
    if(File_OpenForRead(&pFile->file, pPath))
        return 1;

    return MMFile_MapForRead(pFile);
}

static inline void MMFile_Unmap(MMFile *pFile){
#ifdef __linux__
